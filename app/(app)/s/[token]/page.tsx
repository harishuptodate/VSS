...
export default async function TokenView({
	...
	if (link.expiresAt && link.expiresAt <= new Date())
		return <div className="p-4">This link has expired</div>;

	if (link.visibility === 'PRIVATE') {
		if (!user)
			return <div className="p-4">Sign in to view this private link.</div>;
		const allowed = link.emails.some(
			(e) => e.email.toLowerCase() === (user.email ?? '').toLowerCase(),
		);
		if (!allowed)
			return (
				<div className="p-4">Your account is not authorized for this link.</div>
			);
	}
